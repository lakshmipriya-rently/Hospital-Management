...
RSpec.describe User, type: :model do
  ...
  context "when callbacks" do
   ...
   describe '.patient' do
    it 'returns only users with userable_type = Patient' do
      patient = create(:user, userable_type: 'Patient')
      doctor = create(:user, userable_type: 'Doctor')

      result = described_class.patient
  ...
  describe '.doctor' do
    it 'returns only users with userable_type = Doctor' do
      doctor = create(:user, userable_type: 'Doctor')
      patient = create(:user, userable_type: 'Patient')

      result = described_class.doctor
